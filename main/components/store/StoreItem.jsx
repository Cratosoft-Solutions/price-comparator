...
import { PRODUCT_SAVE_CONFIRM_ACTION,CATEGORY_TYPES, IMAGE_LOADED_CONFIRM_ACTION,IMAGE_MAX_PASSED, IMAGE_FAILED_CONFIRM_ACTION, GENERAL_UKNOWN_ERROR, GENERAL_SUCCESS_PROCESS, CURRENCY_LIST, IMAGE_PRODUCT_STORE_DIMENSION, CATEGORY_DEFAULT_LIST, DROPDOWN_NEED_TB_SELECTED, GENERAL_YESNO     } from '@utils/constants';
...
const StoreItem = ({ editMode=false, product, onCloseFunction }) => {
  ...
  return (
    <>
      <form method='POST' onSubmit={confirmAction} className={`relative bg-white mt-4 lg:mt-0 p-4 lg:bg-gray-100 lg:pl-6 lg:pr-6 ${
        expandedNavBar && !editMode? "ml-0 lg:ml-52" : `ml-0 lg:ml-${editMode?0:10}`
      }`}>  {<IoIosCloseCircleOutline className="absolute top-2 right-2 w-8 h-8" color='red' onClick={onCloseFunction}/>}
                    ...
                    <div className="lg:col-span-2">
                        <div className="grid text-sm grid-cols-1 md:grid-cols-5">
                             ...
                             <div className="md:col-span-5 relative">
                                <DropDownList additionalClass="rounded-t-lg" onSelectValue={setCategory} values={categoryList} type="text" required name="category" id="category"  currentValue={category} onChange={e => {setCategory(e.target.value);}} />
                                <div className="labelsconfigurationwithvalue text-gray-600 text-sm">Sección en que se mostrará el producto.</div>
                            </div>

                            <div className="md:col-span-5 relative">
                                <input type="text" required  name="name" id="name" className="inputconfiguration focus:font-bold focus:orange_gradient h-16 pt-2 border px-4 w-full bg-white shadow text-base text-gray-800 border border-gray-300" value={name} onChange={e => {setName(e.target.value);}}/>
                            ...
                            <div className="md:col-span-5 relative">
                                <CurrencyInput type="stock" value={stock}  onChange={e => {setStock(e.target.value);}} />
                                <div className={`${stock?'labelsconfigurationwithvalue':'labelsconfiguration'} text-gray-600 text-sm`}>Cantidad en stock.</div>
                            </div>
                            ...
                            <div className="md:col-span-5 relative ">
                                <DropDownList  onSelectValue={setNegotiable} values={GENERAL_YESNO} type="text" required name="negotiable" id="negotiable" currentValue={negotiable} onChange={e => {setNegotiable(e.target.value);}}/>
                                <div className="labelsconfigurationwithvalue text-gray-600 text-sm">¿Precio Negociable?.</div>
                            </div>

                            <div className="md:col-span-5 relative ">
                                <DropDownList additionalClass={category =="PRODUCT"?"rounded-b-lg":""} onSelectValue={setCurrency} values={CURRENCY_LIST} type="text" required name="currency" id="currency"   currentValue={currency} onChange={e => {setCurrency(e.target.value);}}/>
                                <div className="labelsconfigurationwithvalue text-gray-600 text-sm">Seleccione la moneda.</div>
                            </div>


                    {category =="CAR" && <CarInfo onChangeValues={setInternalCarInfo} carInfo={carInfo}/>}

                            

                            {images.length <= 4 && 
                                <div className="md:col-span-5 relative mt-4">
                                    <div className="labelsconfigurationwithvalue text-gray-600 text-sm">Seleccione las imágenes del item <p className='font-bold inline-flex'>(Máximo 5 por item)</p>.</div>
                        ...
                        <div className="md:col-span-5 text-right mt-4">
                                <button type="submit" className="inline black_btn">{editMode?"Actualizar":"Guardar"}</button>
                        </div>
    ...
    {showImage && <UploadedImage image={images[selectedImage]} onCloseFunction={setShowImage} onDeleteImage={deleteImage} index={selectedImage}/>} 
