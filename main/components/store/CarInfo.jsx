        ...
        <div className="grid grid-cols-1 lg:grid-cols-3">
          {CARD_EQUIPMENT.map((element, index) => (
            <div key={index} className="flex items-center mb-4">
              <input
                id={`card-equipment-${element.value}`}
                key={`card-equipment-${element.value}`}
                type="checkbox"
                checked={
                  carInfo.equipment.filter(
                    (selectedElement) => selectedElement == element.value
                  ).length > 0
                }
                className="w-4 h-4 accent-gray-900"
                onChange={() => {
                  fieldSelectedValue(element.value, "equipment");
                }}
              />
              <label
