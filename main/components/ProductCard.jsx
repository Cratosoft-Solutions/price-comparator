...
const ProductCard = ({ logo, product, index, adminMode, callBackFunction }) => {
  ...
  return (
    ...
    <div
      ...
      className={` hover:cursor-pointer product-image flex items-center justify-center block bg-dark-surface rounded-lg p-2 shadow shadow-dark-surface/50 border border-dark-border hover:border-accent-primary/30 transition-all`}
    >
     <div key={`CARD-${index}`} className="flex flex-col w-32 h-64 lg:w-48 lg:h-80 gap-2">
      ...
      <div className="h-20 flex items-start justify-center font-extrabold text-dark-text">
        {product.productName}
      ...
      <div className="rh-10 lg:h-4 flex-row lg:flex inline gap-2 items-center justify-center mt-2 lg:mt-0">        
            {Boolean(product.formatedEspecialPrice && 
             product.formatedEspecialPrice != 0) && (
              <div className="lg:inline text-center text-dark-muted">
        ...
        <div className="lg:inline text-center text-dark-text">
            <span className='text-xs'>{product.currency == "CRC" ? "₡" : product.currency}</span>
