...
import { fetchWithTimeout, formatKeyForStorage } from "@utils/functions";
  ...
  const fetchProducts = async (e) => {
    e.preventDefault();
     ...
     const products = window.sessionStorage.getItem(key);
     if(products != null){
      JSON.parse(products).forEach(element => {
          processIndividualResponse(element);
        });
     }else{
        STORE_BY_CATEGORY.filter((item)=> item.category == selectedCategory)[0].stores.map((storeID)=>{
          fetchWithTimeout(`/api/search/${storeID}/${searchText.replace(" ", "+")}`).then(r => r.json()).then((data)=> processIndividualResponse(data)).catch((error)=>processIndividualError(error));
        }) 
     }
}
